...
function MenuPage() {
  ...
  const normalizeMenu = (item) => {
    ...
    // Ambil harga dari backend (bisa berupa angka, string angka "25000")
    const rawPrice = item.harga ?? item.price ?? 0
    const priceValue = typeof rawPrice === 'number'
      ? rawPrice
      : parseInt(String(rawPrice).replace(/\D/g, ''), 10) || 0

    const initialStok = Number(item.stok ?? item.availability ?? 0);
    ...
    return {
      ...
      name,
      nama_menu: name,
      price: priceValue, // Pass raw number, let MenuCard handle formatting
      priceValue,
      stok: Math.max(0, initialStok - cartQty), // Stok berkurang sesuai keranjang
      stok_asli: initialStok, // Referensi stok asli
